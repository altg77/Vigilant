...
function carregarRiscos() {
  ...
  // Overlay de Cadastro de Risco (movido para cá para melhor organização)
      ...
      novoRiscoForm.addEventListener("submit", function(event) {
        ...
        if (window.location.pathname.includes('riscos.html')) {
          ...
          if (riscosLista) {
            ...
            setorCell.innerHTML = `<img src="/assets/images (35).jpg" /><p>${novoRisco.setor}</p>`;
            codigoCell.textContent = novoRisco.codigo;
            maquinaCell.innerHTML = `<p>${novoRisco.maquina}</p>`;
            descricaoCell.textContent = novoRisco.descricao;
            criticidadeCell.innerHTML = `<span class="status ${novoRisco.criticidade === 'Alto' ? 'completed' : novoRisco.criticidade === 'Médio' ? 'pending' : 'critical'}">${novoRisco.criticidade}</span>`;
            dataCell.textContent = novoRisco.data;
            statusCell.innerHTML = `<span class="status ${novoRisco.status === 'Concluído' ? 'completed' : novoRisco.status === 'Em analise' ? 'pending' : 'critical'}">${novoRisco.status}</span>`;
            ...
            deletarButton.classList.add('btn-deletar');
            deletarButton.addEventListener('click', () => {
              confirmarDelecaoRisco(novoRisco.codigo);
            });
            acoesCell.appendChild(deletarButton);
          }
        } else if (window.location.pathname.includes('index.html')) {
